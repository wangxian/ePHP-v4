...
function dump($vars, $label=null, $vardump=true)
{
    ...
    if($vardump)
    {
        ob_start();var_dump($vars);$output = ob_get_clean();
...
function C($name='', $config_name='main', $value=null)
{
    ...
    {//获取值
		...
		else if( array_key_exists($name, $_config[$config_name]) ) return $_config[$config_name][$name];
	    else if( strpos($name,'.') )
	    {
	        $array = explode('.',$name );
...
function __autoload($classname)
{
	...
	{#加载第三方库，位置在exts/下
		...
		if(in_array($classname,$system_class)) include FW_PATH.'/libraries/'.$classname.'.class.php';
		else
		{
			$classname = str_replace( '_' , '/', $classname);
			if( file_exists(APP_PATH.'/exts/'.$classname.'.class.php') ) include APP_PATH.'/exts/'.$classname.'.class.php';
...
function M($model_name, $db_config_name='')
{
	...
	else $obj = $GLOBALS['ePHP']['M_QM'][$model_name];
...
function QM($model_name, $db_config_name='default')
{
	...
	else $obj = $GLOBALS['ePHP']['M_QM'][$model_name.'Model'];
	return $obj;
}
...
function show_error($message, $url='', $wait=6)
{
	...
	$tpl = C('tpl_error');
	if(! $tpl ) include FW_PATH.'/tpl/error.tpl.php';
	else  include APP_PATH.'/views/public/'.$tpl;
	exit;
}
...
function show_success($message, $url='', $wait=6)
{
	...
	$tpl = C('tpl_success');
	if(! $tpl ) include FW_PATH.'/tpl/success.tpl.php';
	else  include APP_PATH.'/views/public/'.$tpl;
	exit;
}
...
/**
 * 获取url中的片段
 * 如 url:/index.php/user/info/12.html,getp(3)的值为12
 * @param integer $pos 获取url片段的位置($pos>=1)
 * @param string $default
 * @return mixed
 */
function getp($pos,$default='')
{
	...
	{//only first time
		if(!empty($_SERVER['PATH_INFO']))
		{
			...
			if( $html_url_subffix ) $_SERVER['PATH_INFO'] = str_replace($html_url_subffix, '', $_SERVER['PATH_INFO']);
			if(C('url_type') == 'NODIR') $_SERVER['PATH_INFO'] = str_replace('-', '/', $_SERVER['PATH_INFO']);
			$url_part = explode('/', substr($_SERVER['PATH_INFO'],1) );//dump($url_part);
		}
...
class app
{
	...
	public function run()
	{
		try
		{
			...
			else if(! C('show_errors') ) show_404();
	...
	public function __get($key)
	{
		...
		switch ($key)
		{
			...
			case substr($key,0,5) == 'model':
				if($key == 'model') return $this->model = new model;
				else
				{
...
class view
{
	 ...
	 */
	public function display($file='',$expire=-1) { $this->render($file,$expire); }
	 ...
	 * @param mixed  $value
	 */
    public function assign($name,$value) { $this->vars[$name] = $value; }
	 ...
	 */
	public function layout($file='', $expire=-1) { $this->render($file, $expire, true); }

	/**
	 * 视图继承
	 * @param string $file
	 */
	protected function _extends($file) { $this->_include($file); }
	...
	public function _include($file, $__vars=null, $layout_block=false, $return=false)
    {
		...
		{
			ob_start();include APP_PATH . '/views/'. $this->__filename($file);
			return ob_get_clean();
		}
		else include APP_PATH . '/views/'. $this->__filename($file);
    }
}
...
#设置资源目录的位置
if($config['assets_dir'] == '') $config['assets_dir'] = $base_url.'/assets'; //如果没有配置assets_dir，则采用当前目下的assets/作为资源目录
define('SOURCE', $config['assets_dir']);
